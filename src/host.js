...
/**
 * Display names for the clips the plugin SHIPS, keyed by filename.
 *
 * Both shipped clips need one: `assets/boot.mp4` is a historical filename that
 * says nothing about its content, and `videos/deepseek-brand-intro.mp4` reads as
 * a slug. Without labels the picker showed three opaque rows across three
 * different source badges, and the clip that WAS bundled looked absent.
 *
 * This map is the only place the plugin claims to know what a shipped file
 * contains. Swap a shipped clip and update its entry here — that is the whole
 * contract.
 */
const SHIPPED_LABELS = {
  'boot.mp4': 'DeepSeek 赛博朋克片头',
  'deepseek-brand-intro.mp4': 'DeepSeek 品牌片头',
}

function displayName(source, fileName) {
  // Only files the plugin itself ships get a label; a file the user dropped in
  // keeps its own name, because the plugin cannot know what it contains.
  if (source === 'bundled' || source === 'shipped') {
    const label = SHIPPED_LABELS[fileName.toLowerCase()]
